    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(4,3,12,.9)', backdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.bg2, border: `1px solid rgba(79,140,255,.22)`, borderRadius: 20, padding: '2rem', width: '100%', maxWidth: 380, boxShadow: '0 40px 80px rgba(0,0,0,.7)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: '1.1rem' }}>Buy Shares</div>
        ...
        <a href="/dashboard/settings" style={{ display: 'block', textAlign: 'center', padding: '.7rem', borderRadius: 10, background: C.blue, color: '#fff', fontSize: '.88rem', fontWeight: 700, textDecoration: 'none' }}>
          Connect Broker →
        </a>
      </div>
    </div>
...
export default function QuantLabPage() {
  ...
  const [chatMsgs, setChatMsgs]         = useState<{role:'user'|'ai';text:string}[]>([
    { role: 'ai', text: "Hi! I'm your quant assistant. I can help you design alpha models, interpret backtest results, and optimize your strategy. What would you like to work on?" },
  ])
  ...
  const handleSave = () => { setSaved(true); addTerm(`✓ ${activeFile} saved`) }
  ...
  async function sendChat() {
    ...
    try {
      ...
      setChatMsgs(p => [...p, { role: 'ai', text: d.content ?? d.message ?? d.text ?? "I'm here to help with strategy design and alpha research. What would you like to improve?" }])
    } catch {
  ...
  return (
    <>
      {/* Full-bleed layout that escapes the quant-content padding */}
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 80px)', margin: '-1.5rem', width: 'calc(100% + 3rem)', background: C.bg, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '.55rem', padding: '.45rem .85rem', borderBottom: `1px solid ${C.border}`, background: C.bg2, flexShrink: 0 }}>
          ...
          <input value={agentName} onChange={e => setAgentName(e.target.value)} style={{ background: 'transparent', border: 'none', outline: 'none', fontWeight: 700, fontSize: '.88rem', color: C.white, minWidth: 120, maxWidth: 240 }} />
          ...
          <button onClick={() => void runBacktest()} disabled={btLoading} style={{ display: 'flex', alignItems: 'center', gap: '.38rem', padding: '.35rem .8rem', borderRadius: 7, border: 'none', background: btLoading ? `${C.blue}55` : C.blue, color: '#fff', fontFamily: 'var(--font-mono)', fontSize: '.6rem', fontWeight: 700, cursor: btLoading ? 'not-allowed' : 'pointer' }}>
            {btLoading
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '.9rem' }}>
              ...
              {rightTab === 'backtest' && (
                <div>
                  ...
                  <button onClick={() => void runBacktest()} disabled={btLoading} style={{ width: '100%', padding: '.58rem', borderRadius: 9, border: 'none', background: btLoading ? `${C.blue}55` : C.blue, color: '#fff', fontFamily: 'var(--font-mono)', fontSize: '.7rem', fontWeight: 700, cursor: btLoading ? 'not-allowed' : 'pointer', marginBottom: '.85rem' }}>
              ...
              {rightTab === 'chat' && (
                <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '.38rem', borderTop: `1px solid ${C.border}`, paddingTop: '.6rem' }}>
                    <textarea value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendChat() } }} placeholder="Ask about your strategy…" rows={2} style={{ flex: 1, background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 8, padding: '.45rem .65rem', color: C.text, fontFamily: 'var(--font-mono)', fontSize: '.66rem', outline: 'none', resize: 'none', lineHeight: 1.55 }} />
                    <button onClick={() => void sendChat()} disabled={!chatInput.trim() || chatLoading} style={{ padding: '.45rem .6rem', borderRadius: 8, background: chatInput.trim() ? C.blue : `${C.blue}40`, border: 'none', color: '#fff', cursor: chatInput.trim() ? 'pointer' : 'default', alignSelf: 'flex-end' }}>
                    ...
                    !published ? (
                      ...
                      <div>
                        ...
                        <div style={{ padding: '.85rem .9rem', background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 12 }}>
                          ...
                          <button onClick={() => setShowInvestModal(true)} style={{ width: '100%', padding: '.58rem', borderRadius: 9, border: 'none', background: C.mint, color: '#fff', fontSize: '.8rem', fontWeight: 800, cursor: 'pointer', letterSpacing: '-.01em' }}>
