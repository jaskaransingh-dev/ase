...
// .wrangler/tmp/pages-CzmF3e/functionsWorker-0.22116090071286132.mjs
var __defProp2 = Object.defineProperty;
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
var cachedColumns = null;
var cachedColumnsAtMs = 0;
...
function getInsertedId(result) {
  const candidate = result?.meta?.last_row_id ?? result?.lastInsertRowid ?? result?.lastInsertRowId ?? null;
  const id = Number(candidate);
  return Number.isFinite(id) ? id : null;
}
__name(getInsertedId, "getInsertedId");
__name2(getInsertedId, "getInsertedId");
...
async function getWaitlistColumns(env) {
  const now = Date.now();
  if (cachedColumns && now - cachedColumnsAtMs < 6e4) return cachedColumns;
...
__name(getWaitlistColumns, "getWaitlistColumns");
__name2(getWaitlistColumns, "getWaitlistColumns");
...
__name(sendSendGridConfirmationEmail, "sendSendGridConfirmationEmail");
__name2(sendSendGridConfirmationEmail, "sendSendGridConfirmationEmail");
async function insertWaitlistUser(env, { email, ip, userAgent }) {
  const cols = await getWaitlistColumns(env).catch(() => /* @__PURE__ */ new Set());
  ...
  if (hasIp && hasUserAgent) {
    ...
    ).bind(email, ip, userAgent).run();
  }
  return env.DB.prepare("INSERT INTO waitlist_users (email) VALUES (?)").bind(email).run();
}
__name(insertWaitlistUser, "insertWaitlistUser");
__name2(insertWaitlistUser, "insertWaitlistUser");
...
__name(updateConfirmationSuccess, "updateConfirmationSuccess");
__name2(updateConfirmationSuccess, "updateConfirmationSuccess");
...
__name(updateConfirmationError, "updateConfirmationError");
__name2(updateConfirmationError, "updateConfirmationError");
...
async function onRequestGet(context) {
  ...
  const cols = await getWaitlistColumns(env).catch(() => /* @__PURE__ */ new Set());
  const selectCols = ["id", "email", "created_at"].concat(cols.has("confirmation_sent_at") ? ["confirmation_sent_at"] : []).concat(cols.has("confirmation_error") ? ["confirmation_error"] : []);
...
// ../../../.nvm/versions/node/v25.8.0/lib/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../.nvm/versions/node/v25.8.0/lib/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-UHRoS8/middleware-insertion-facade.js
...
// ../../../.nvm/versions/node/v25.8.0/lib/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-UHRoS8/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.22116090071286132.js.map
